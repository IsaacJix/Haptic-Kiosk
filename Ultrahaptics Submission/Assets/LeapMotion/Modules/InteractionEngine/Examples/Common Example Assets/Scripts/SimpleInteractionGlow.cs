...
public class SimpleInteractionGlow : MonoBehaviour {
  ...
  private InteractionBehaviour _intObj;
    ...
    public List<GameObject> ActivateOnPress, DeactivateOnPress, ActivateOnHover, DeactivateOnHover;
    ...
    //if (_material != null) 
    {
      ...
      if (_intObj.isPrimaryHovered && usePrimaryHover) {
        targetColor = primaryHoverColor;
                ...
                foreach (GameObject obj in ActivateOnHover)
                {
                    obj.SetActive(true);
                }
                foreach (GameObject obj in DeactivateOnHover)
                {
                    obj.SetActive(false);
                }
            }
      ...
      if (_intObj is InteractionButton && (_intObj as InteractionButton).isPressed) {
        targetColor = pressedColor;
                ...
                foreach (GameObject obj in ActivateOnPress)
                {
                    obj.SetActive(true);
                }
                foreach (GameObject obj in DeactivateOnPress)
                {
                    obj.SetActive(false);
                }
      }
      ...
      if(_material != null)
      {
          _material.color = Color.Lerp(_material.color, targetColor, 30F * Time.deltaTime);
            ...
            {
                mat.color = Color.Lerp(mat.color, targetColor, 30F * Time.deltaTime);
            }

            foreach (Renderer rend in renderers)
            {
                rend.material = materials[renderers.IndexOf(rend)];
            }
    }
  }
